var rumoHeadingsmap_bio_headings = {
	...
	ratonClick: function(e,ind){
		if(e.button == 2){
			...
			if(!ind.view.getItemAtIndex(ind.currentIndex).firstChild.firstChild.getAttribute('value') && !rumoHeadingsmap_bio_headings.slides())
	...
	numeraCap: function(arbol,indice){
		for(var i = 0; i < arbol.childNodes.length; i++){
			if(arbol.childNodes[i].tagName.toLowerCase() == "treeitem")
				indice = i + 1;
			if(arbol.childNodes[i].tagName.toLowerCase() == "treecell")
				arbol.childNodes[i].setAttribute("label", indice  + " - " + arbol.childNodes[i].getAttribute("label"));
			if(arbol.childNodes[i].childNodes.length > 0)
				rumoHeadingsmap_bio_headings.numeraCap(arbol.childNodes[i],indice);
		}
	},
	encabezados: function(){
		try{
			...
			var wHead = new Array(false,false,false,false,false);
			var maxHead = 0;
			var lim = 2;
			//El siguiente condicional es para presentaciones con secciones en rumo_slidy
			if(rumoHeadingsmap_bio_headings.getElementsByClassName(window.content.document, 'div', 'subtema').length)
				lim = 3;
			//Lo siguiente es para poder decidir si el primero es error cuando no sea h1
			var totales = 0;
			var sigue = false;
			var rumoslide = false;
			var wh = rumoHeadingsmap_bio_headings.slides();
			if(wh == 'rumoslide')
				rumoslide = true;
			for (var doc_number = 0; doc_number < total; doc_number++) {
				...
				if (encabezados.length > 0) {
					...
					for (var i = 0; i < encabezados.length; i++) {
						if(!rumoslide || (rumoslide && rumoHeadingsmap_bio_headings.getText(encabezados[i]).replace(/\n/g, '').replace(/\t/g, '')) != 'notas'){
	...
	slides: function(){
		...
		var documentos = rumoHeadingsmap_bio_documents.documents();
		for (var doc_number = 0; doc_number < documentos.length; doc_number++) {
			if(documentos[doc_number].contentType == "application/xhtml+xml")
				var scripts = rumoHeadingsmap_bio_xpath.getElementsByTagNames('script',documentos[doc_number]);
			else
				var scripts = rumoHeadingsmap_bio_xpath.resuelvexpath(documentos[doc_number], '//script[@src]');
			for (var i = 0; i < scripts.length; i++) {
				if (scripts[i].getAttribute('src')){
					if (scripts[i].getAttribute('src').indexOf('slidy.js') > -1) 
						wh = 'slidy';
					if (scripts[i].getAttribute('src').indexOf('rumoslide.js') > -1) 
						wh = 'rumoslide';
				}
			}
		}
	...
	headingsAtSameTime: function(act,headings,num,wh,doc) {
		...
		var definitivos = new Array(headings,num,doc);
		return definitivos;
	},
	...
	copyHead: function(ind,tipo){
		...
		if(tipo && (tree.view.getItemAtIndex(ind).firstChild.firstChild.getAttribute('value') || rumoHeadingsmap_bio_headings.slides())){
			var dir = window.content.location.href;
			if(treeActual.parentNode.getAttribute('tooltiptext'))
				if(treeActual.parentNode.getAttribute('tooltiptext').indexOf(dir.substr(0,dir.lastIndexOf('/'))) == -1)
		...
		var wh = rumoHeadingsmap_bio_headings.slides();
		if(wh == 'rumoslide'){
			var indNew = parseFloat(treeActual.currentIndex);
			var lim = 2;
			if(rumoHeadingsmap_bio_headings.getElementsByClassName(window.content.document, 'div', 'subtema').length)
				lim = 3;
			if(treeActual.view.getItemAtIndex(indNew).getAttribute('value') > lim){
				while(!treeActual.view.getItemAtIndex(indNew).getAttribute('class'))
					indNew--;
			}
			indNew = parseFloat(treeActual.view.getItemAtIndex(indNew).getAttribute('class').replace('acc','')) + 1;
			if(lab.indexOf('#') > 0)
				lab = lab.substr(0, lab.indexOf('#')) + '#slide' + indNew;
		}
	...
	selectHead: function(ind){
		if(rumoHeadingsmap_bio_headings.activar){
			...
			var slDest = ind + 1;
//			if(ind >= 0)
//				if(tree.view.getItemAtIndex(ind).getAttribute('open') && tree.view.getItemAtIndex(ind).childNodes.length > 1 && document.getElementById('level' + tree.view.getItemAtIndex(ind).firstChild.getAttribute('class').replace('biohead','') + 'p').checked)
//					document.getElementById('level' + tree.view.getItemAtIndex(ind).firstChild.getAttribute('class').replace('biohead','') + 'p').checked = false;
			if (ind >= 0) {
				...
				if (rumoslide) {
					var indNew = parseFloat(tree.currentIndex);
					var lim = 2;
					//Para presentaciones con secciones se pone el siguiente condicional
					if(rumoHeadingsmap_bio_headings.getElementsByClassName(window.content.document, 'div', 'subtema').length)
						lim = 3;
					if(tree.view.getItemAtIndex(indNew).getAttribute('value') > lim){
						while(!tree.view.getItemAtIndex(indNew).getAttribute('class'))
							indNew--;
					}
					indNew = parseFloat(tree.view.getItemAtIndex(indNew).getAttribute('class').replace('acc',''));
					rumoHeadingsmap_bio_headings.gotoSlide(indNew);
				} else {
					if (slidy) {
						var dir = window.content.location.href.substr(0, window.content.location.href.indexOf('#'));
						dir = dir + '#(' + slDest + ')';
						var slide = jso.slides[jso.slidenum];
						jso.hideSlide(slide);
						jso.slidenum = jso.findSlideNumber(dir);
						slide = jso.slides[jso.slidenum];
						jso.lastShown = null;
						jso.setLocation();
						jso.setVisibilityAllIncremental("hidden");
						jso.setEosStatus(!jso.nextIncrementalItem(jso.lastShown));
						jso.showSlide(slide);
					} else {
						rumoHeadingsmap_bio_headings.cleanPrevioHead();
						...
						if (documentos[rumoHeadingsmap_bio_headings.marco].getElementsByTagName('frameset').length) {
							...
							if (documentos[ruta].getElementsByTagName('iframe').length) {
								...
								contenedores[ruta].scrollTo(rumoHeadingsmap_bio_headings.posicionAbsoluta(headings[ind]).left, rumoHeadingsmap_bio_headings.posicionAbsoluta(headings[ind]).top - 20);
							}
						}else {
							where = [];
	...
	cleanTreeSeleccion: function(el){
		if(rumoHeadingsmap_bio_headings.activar){
			if(document.getElementById('comprobaciones')){
				var trees = document.getElementById('comprobaciones').getElementsByTagName('tree');
				var inicio = 0;
				if(trees.length > 1)
					inicio = 1;
				for (var i = inicio; i < trees.length; i++)
					if((!el || trees[i] != el) && trees[i].view)
						trees[i].view.selection.select(-1);
			}
		}
	...
	gotoSlide: function(wh){
		wh = parseInt(wh);
		var winWrapper = new XPCNativeWrapper(content);
		var jso = winWrapper.wrappedJSObject;
		jso.rumoslide.gotoSlide(wh, 'all');
	},
